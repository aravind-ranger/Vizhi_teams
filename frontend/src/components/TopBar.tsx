...
const TopBar: React.FC<TopBarProps> = ({ onFocusMode, onSidebarToggle }) => {
    ...
    <>
      <header className="sticky top-0 h-[70px] glass z-20 flex flex-wrap items-center justify-between px-4 sm:px-6 mx-4 sm:mx-6 my-2 rounded-2xl transition-all duration-300">
        <div className="flex items-center min-w-0">
          ...
          <h1 className="text-xl font-bold text-text-primary truncate">{getPageTitle()}</h1>
        </div>

        <div className="flex flex-wrap items-center gap-3">
          {/* Attendance Widget */}
          <div className="flex flex-wrap items-center gap-3 bg-gray-100/50 p-2 rounded-xl">
            {isAttLoading ? (
