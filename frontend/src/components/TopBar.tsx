    ...
    const timer = setInterval(() => {
      ...
      if (
        ...
        const diffHours =
          ...
          (1000 * 60 * 60);
        if (diffHours >= 7.75 && diffHours < 8 && !hasNotifiedShiftEnd) {
          // 15 mins before
    ...
    <>
                    ...
                    return todayNotifs.map((n) => (
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <p
                            className={`text-sm font-medium ${!n.is_read ? "text-text-primary" : "text-text-secondary"}`}
                          >
                            {n.title}
                          </p>
                          {!n.is_read && (
            ...
            {showUserMenu && (
              <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-glass rounded-xl shadow-modal border border-border dark:border-white/10 py-1 overflow-hidden z-50">
                  ...
                  <div className="grid grid-cols-1 gap-1">
                    {[
                      ...
                      <button
                        ...
                        className={`flex items-center px-3 py-2 rounded-lg text-xs font-bold transition-all hover:bg-gray-50 dark:hover:bg-white/10 ${user?.availability_status === status.id ? "bg-primary/5 text-primary dark:bg-primary/10 dark:text-white" : "text-text-secondary dark:text-text-secondary dark:hover:text-white"}`}
                      >
                        <div
                          className={`w-2 h-2 rounded-full mr-3 ${status.color}`}
                        />
                        {status.label}
                      </button>
                    ))}
                ...
                <button
                  ...
                  className="w-full flex items-center px-4 py-2.5 text-sm text-text-secondary hover:bg-gray-100 dark:hover:bg-white/10 transition-colors"
                >
      ...
      {showCheckInModal && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          <div
            className="absolute inset-0 bg-text-primary/40 dark:bg-black/60 backdrop-blur-sm"
          ...
          <div className="relative bg-white dark:bg-glass dark:border dark:border-white/10 w-full max-w-md p-10 rounded-[40px] shadow-2xl animate-scale-up">
            <h3 className="text-2xl font-black text-text-primary mb-2 text-center">
              Check-in Location
            </h3>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-glass dark:border dark:border-white/10 w-full max-w-sm p-10 rounded-[40px] shadow-2xl animate-scale-up text-center">
            ...
            <p className="text-text-muted mb-8 font-medium">
              Are you sure you want to check out? All active task timers will be
              stopped.
            </p>
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8">
              {(() => {
                ...
                if (sortedDates.length === 0) {
                  return (
                    <div className="p-20 text-center text-text-muted font-bold">
                  ...
                  <div key={date} className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      {grouped[date].map((n) => (
                        <div
                          key={n.id}
                          className={`p-6 rounded-[24px] border ${n.is_read ? "bg-white dark:bg-white/5 border-border dark:border-white/5" : "bg-primary/5 border-primary/20 shadow-sm shadow-primary/10"}`}
                        >
                          <div className="flex justify-between items-start mb-2">
                            <h4 className="font-black text-text-primary tracking-tight">
                              {n.title}
                            </h4>
