...
import { User, Shield, Key, Mail, Building, Briefcase, Camera, ChevronRight, Lock } from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-slide-up">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-black text-text-primary">Profile Settings</h2>
          <p className="text-sm text-text-muted">Manage your personal information and security</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="glass p-8 rounded-[40px] text-center border-none shadow-sm relative overflow-hidden group">
            ...
            <div className="relative mb-6 inline-block">
              <Avatar name={user?.name || ''} url={user?.avatar_url} size="xl" className="ring-8 ring-white shadow-xl" />
            ...
            <p className="text-[10px] font-bold text-primary uppercase tracking-[0.2em] mb-4">{user?.role}</p>
            <div className="flex justify-center space-x-2">
              <span className="px-3 py-1 bg-success/10 text-success text-[10px] font-black rounded-lg uppercase tracking-widest border border-success/10">Active</span>
              <span className="px-3 py-1 bg-primary/10 text-primary text-[10px] font-black rounded-lg uppercase tracking-widest border border-primary/10">{user?.department || 'Operations'}</span>
            </div>
          </div>

          <div className="glass p-8 rounded-[40px] border-none shadow-sm space-y-4">
            <h4 className="text-[10px] font-black text-text-muted uppercase tracking-[0.2em] mb-4">Security</h4>
            <button 
              onClick={() => setShowPasswordModal(true)}
              className="w-full flex items-center justify-between p-4 bg-gray-50 rounded-2xl group hover:bg-primary/5 transition-all"
            >
              <div className="flex items-center space-x-4">
                <div className="p-2 bg-white rounded-xl shadow-sm group-hover:text-primary transition-colors">
                  <Key className="w-4 h-4" />
                </div>
                <span className="text-sm font-bold text-text-secondary group-hover:text-primary transition-colors">Change Password</span>
              </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="glass p-10 rounded-[40px] border-none shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-2">
                ...
                <div className="flex items-center space-x-4 p-5 bg-gray-50 rounded-2xl border border-gray-100">
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center space-x-4 p-5 bg-gray-50 rounded-2xl border border-gray-100">
                  ...
                  <span className="text-sm font-bold text-text-primary">{user?.department || 'Engineering'}</span>
                </div>
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black text-text-muted uppercase tracking-widest ml-1">Role / Designation</label>
                <div className="flex items-center space-x-4 p-5 bg-gray-50 rounded-2xl border border-gray-100">
                  <Briefcase className="w-4 h-4 text-text-muted" />
                  <span className="text-sm font-bold text-text-primary capitalize">{user?.role}</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="glass p-10 rounded-[40px] border-none shadow-sm">
            ...
            <div className="flex items-center justify-between p-6 bg-success/5 rounded-[32px] border border-success/10">
              <div className="flex items-center space-x-6">
                <div className="w-14 h-14 bg-success/10 rounded-2xl flex items-center justify-center">
                  <Shield className="w-7 h-7 text-success" />
                </div>
                <div>
                  <p className="text-base font-black text-text-primary">Verified Employee Account</p>
                  <p className="text-xs text-text-muted font-medium">Your account is fully verified and secure.</p>
                </div>
              </div>
              <span className="px-4 py-2 bg-success text-white text-[10px] font-black rounded-xl uppercase tracking-widest">Verified</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {showPasswordModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => !isUpdating && setShowPasswordModal(false)} />
          <div className="relative bg-white w-full max-w-md rounded-[40px] p-10 shadow-2xl animate-scale-up">
            ...
            <p className="text-text-muted text-sm font-medium mb-8">Secure your account by choosing a strong password.</p>
            
            <form onSubmit={handleChangePassword} className="space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-text-muted uppercase tracking-widest ml-1">Old Password</label>
                <input 
