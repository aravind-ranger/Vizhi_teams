  ...
  const fetchHistory = async () => {
    ...
    try {
      ...
      const allAttData = attSnap.docs.map(doc => {
        const item = doc.data();
        const date = item.created_at?.toDate ? item.created_at.toDate() : new Date(item.check_in || item.created_at);
        return {
          id: doc.id,
      ...
      const attData = allAttData.filter(item => {
      ...
      // Calculate Stats
      const totalDaysPresent = attData.length;
      const totalMinutes = attData.reduce((acc: number, curr) => acc + (curr.duration_minutes || 0), 0);
      const avgHours = totalDaysPresent > 0 ? (totalMinutes / totalDaysPresent / 60) : 0;
      ...
      const daysSoFar = eachDayOfInterval({ start: monthStart, end: endOfCalculation });
      const weekDaysNeeded = daysSoFar.filter(d => d.getDay() !== 0 && d.getDay() !== 6).length;

      setStats({
        presence_pct: Math.round((totalDaysPresent / weekDaysNeeded) * 100) || 0,
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        ...
        <div className="flex items-center space-x-2">
          ...
          <div className="glass px-6 py-3 rounded-2xl flex items-center space-x-3 border-none shadow-sm">
            ...
            <span className="text-sm font-black text-text-secondary uppercase tracking-widest">{format(selectedDate, 'MMMM yyyy')}</span>
          </div>
          <button 
          ...
          <div className="glass p-8 rounded-[40px] border-none shadow-sm">
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                ...
                <span className="text-lg font-black text-primary">{stats.presence_pct}%</span>
              </div>
              <ProgressBar progress={stats.presence_pct} className="h-2 rounded-full" />
              <div className="grid grid-cols-2 gap-4 pt-4">
                <div className="p-4 bg-gray-50 dark:bg-white/5 rounded-2xl">
                  <p className="text-[10px] font-black text-text-muted uppercase tracking-widest mb-1">Total Days</p>
                  <p className="text-xl font-black text-text-primary">{stats.total_days} / {stats.total_needed}</p>
                </div>
                <div className="p-4 bg-gray-50 dark:bg-white/5 rounded-2xl">
                  <p className="text-[10px] font-black text-text-muted uppercase tracking-widest mb-1">Avg. Hours</p>
                  <p className="text-xl font-black text-text-primary">{stats.avg_hours}h</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="glass p-10 rounded-[40px] border-none shadow-sm h-full">
            <div className="flex justify-between items-center mb-10">
              ...
              <div className="flex items-center space-x-6 text-xs font-bold text-text-muted">
                ...
                <div className="flex items-center"><div className="w-2 h-2 rounded-full bg-purple-500 mr-2" /> Holiday</div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-7 gap-4">
              ...
              {days.map((day, i) => {
                ...
                return (
                  <div
                    key={i}
                    className={`aspect-square rounded-3xl flex flex-col items-center justify-center relative group transition-all cursor-pointer ${isToday ? 'bg-primary text-white shadow-lg shadow-primary/25 scale-105 z-10' :
                      dayLog ? 'bg-success/10 dark:bg-success/20' : 
                      isSunday ? 'bg-purple-50 dark:bg-purple-900/20' :
                  ...
                  >
                    ...
                    <div className="absolute bottom-2 flex space-x-1">
                      {dayLog && <div className="w-1.5 h-1.5 rounded-full bg-success" />}
                      {leaves.some(l => {
                        const from = l.from_date?.toDate ? l.from_date.toDate() : new Date(l.from_date);
                        const to = l.to_date?.toDate ? l.to_date.toDate() : new Date(l.to_date);
                        const d = new Date(day);
                        d.setHours(0, 0, 0, 0);
                        return d >= from && d <= to;
                      }) && <div className="w-1.5 h-1.5 rounded-full bg-warning" />}
                      {isSunday && <div className="w-1.5 h-1.5 rounded-full bg-purple-500" />}
                      {!dayLog && !isToday && day.getDay() !== 0 && day.getDay() !== 6 && day < new Date() && !leaves.some(l => {
                        const from = l.from_date?.toDate ? l.from_date.toDate() : new Date(l.from_date);
                        const to = l.to_date?.toDate ? l.to_date.toDate() : new Date(l.to_date);
                        const d = new Date(day);
                        d.setHours(0, 0, 0, 0);
              ...
              <div className="space-y-4">
                {isLoading ? [1, 2].map(i => <div key={i} className="skeleton h-16 rounded-2xl" />) :
                  history.slice(0, 5).map((log, i) => (
                    <div key={i} className="flex items-center justify-between p-5 bg-gray-50 dark:bg-white/5 rounded-2xl hover:bg-white dark:hover:bg-white/10 transition-all">
                      <div className="flex items-center space-x-6">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] font-bold text-text-muted uppercase tracking-widest mt-0.5">Shift Logged</p>
                        </div>
                      </div>
