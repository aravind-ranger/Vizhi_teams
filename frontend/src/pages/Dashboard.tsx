...
const Dashboard: React.FC = () => {
  ...
  const statCards = [
    ...
    },
    {
      ...
      color: "text-success",
      bg: "bg-success/10",
      action: () => setShowPresentModal(true),
    },
    {
      label: "Absent",
      ...
      color: "text-danger",
      bg: "bg-danger/10",
      action: () => setShowAbsentModal(true),
    },
  ];
          ...
          <div className="bg-gray-50 dark:bg-white/5 px-2 py-1.5 rounded-2xl flex items-center space-x-1 border border-gray-100 dark:border-white/5 shadow-sm">
            {[
              ...
              >
                ...
                <span
                  className={`text-[10px] font-black uppercase tracking-widest ${currentStatus === s.id ? "text-slate-900 dark:text-white" : "text-text-secondary"}`}
                >
                  {s.label}
                </span>
              </button>
        ...
        <div className="glass p-8 rounded-[40px] border-none shadow-sm bg-rose-50/50 dark:bg-rose-900/10 border-rose-100/50 dark:border-rose-900/20">
          ...
          <div className="flex flex-wrap gap-4">
            {absentEmployees.map((emp) => (
              <div
                key={emp.id}
                className="flex items-center space-x-3 bg-white dark:bg-white/5 p-3 rounded-2xl shadow-sm border border-rose-100/50 dark:border-rose-900/20"
              >
                ...
                <div>
                  ...
                  <p className="text-[9px] font-black text-rose-500 dark:text-rose-400 uppercase tracking-tighter">
                    No Check-in
                  </p>
                </div>
              </div>
            ...
            <div className="flex justify-between items-center mb-10 relative z-10">
              <div>
                ...
                <p className="text-sm text-text-muted font-medium">
                  Synchronized with local server time
                </p>
              </div>
              <div className="text-right">
                ...
                <p className="text-[10px] text-text-muted uppercase font-black tracking-[0.3em]">
            ...
            <div className="relative z-10">
              {isAttLoading ? (
                ...
                <div className="bg-success/5 dark:bg-success/10 border border-success/20 dark:border-success/30 rounded-[32px] p-12 text-center">
                  ...
                  <h4 className="text-3xl font-black text-success mb-2">
                    Shift Completed!
                  </h4>
                  <p className="text-text-secondary font-bold mb-8">
                    You've done a great job today. See you tomorrow!
                  </p>
                  <div className="flex justify-center space-x-12">
                    <div>
                      ...
                      <p className="text-3xl font-black">
                        {(attendance.duration_minutes / 60).toFixed(1)}h
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="text-3xl font-black">
                        {safeFormat(attendance.check_out, "HH:mm")}
                      </p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-10">
                  <div className="flex flex-col md:flex-row justify-between items-center gap-8">
                    <div className="text-center md:text-left">
                      ...
                      <p className="text-3xl font-black">
                        {safeFormat(attendance.check_in, "HH:mm a")}
                      </p>
                    </div>
                    <div
                      className={`text-center p-8 rounded-[32px] border min-w-[280px] transition-all ${attendance.is_paused ? "bg-amber-500/10 border-amber-500/20" : "bg-primary/5 border-primary/10"}`}
                    >
                      <p
                        className={`text-[10px] uppercase font-black tracking-widest mb-2 ${attendance.is_paused ? "text-amber-500" : "text-primary"}`}
                      >
                      ...
                      {timeLeft !== null && (
                        <div className="flex items-center justify-center space-x-3">
                          <p
                            className={`text-5xl font-black tracking-tighter tabular-nums ${attendance.is_paused ? "text-amber-500" : timeLeft < 3600 ? "text-danger" : "text-text-primary"}`}
                          >
                  ...
                  <div className="space-y-3">
                    ...
                    <ProgressBar
                      progress={Math.min(
                        100,
                        (1 - (timeLeft || 0) / (8 * 3600)) * 100,
                      )}
                      className="h-4 rounded-full"
                    />
                  </div>
                  <div
                    className={`flex items-center space-x-3 ${attendance.is_paused ? "text-amber-500" : "text-success animate-pulse"}`}
                  >
                    <div
                      className={`w-2 h-2 rounded-full ${attendance.is_paused ? "bg-amber-500" : "bg-success"}`}
                    />
          ...
          <div className="glass p-8 rounded-[32px] border-none shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                >
                  <div
                    className={`p-4 rounded-2xl mb-4 group-hover:scale-110 transition-transform ${action.color} dark:bg-white/10 dark:text-white group-hover:bg-white dark:group-hover:bg-primary`}
                  >
          ...
          <div className="relative bg-white dark:bg-glass dark:border dark:border-border w-full max-w-lg rounded-[40px] shadow-2xl animate-scale-up overflow-hidden">
            <div className="p-8 border-b border-gray-100 dark:border-border flex justify-between items-center bg-gray-50/50 dark:bg-transparent">
              <h3 className="text-xl font-black text-text-primary">
                Select Dashboard Events
              </h3>
              <button
                onClick={() => setShowEventPicker(false)}
                className="p-2 hover:bg-gray-200 dark:hover:bg-white/10 rounded-full transition-colors text-text-muted"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-8 max-h-[60vh] overflow-y-auto space-y-4">
              <p className="text-xs font-bold text-text-muted uppercase tracking-widest mb-4">
                Select events to display on dashboard
              </p>
              {allEvents.length === 0 ? (
          ...
          <div className="relative bg-white dark:bg-glass dark:border dark:border-border w-full max-w-md rounded-[40px] shadow-2xl animate-scale-up overflow-hidden">
            <div
              className={`h-3 w-full ${
                selectedEventDetail.type === "holiday"
                  ? "bg-rose-500"
                  : selectedEventDetail.type === "meeting"
                    ? "bg-indigo-500"
                    : "bg-amber-500"
              }`}
            />
            <div className="p-10">
              <div className="flex justify-between items-start mb-6">
                <div>
                  ...
                  <h3 className="text-2xl font-black text-text-primary mt-4 tracking-tight">
                  ...
                  {user?.role === "admin" && (
                    <button
                      onClick={async () => {
                        if (
                          window.confirm(
                            "Are you sure you want to remove this from the dashboard?",
                          )
                        ) {
                          try {
                            await updateDoc(
